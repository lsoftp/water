...
void QuTab::reg()
{
    ...
    t.cname="";
    t.qname=ui->comboBox_5->currentText();
    t.tr.isdilute=0;
    t.tr.status=0;
    //t.combinetestname="";
    ...
    db.delQubyid(g_current_index,t.no);
    int j=0;
...
{
    QSqlQueryModel sqm;
    db.getQuNo(sqm, g_current_index);
...
void QuTab::clearBtn()
{
    for(int i=0;i<g_item_num;i++)
    {
        pb1[i]->setChecked(false);
    }
}

void QuTab::refreshtable()
{
    QSqlQueryModel sqm;
    db.getQu(sqm,::g_current_index);
    ...
    for(int i=0;i<sqm.rowCount();i++)
    {
        ...
        tw->setItem(i,0,new QTableWidgetItem(QString("Q%1").arg(ts)));
        tw->setItem(i,1,new QTableWidgetItem(sqm.record(i).value("Pos").toString()));
        tw->setItem(i,2,new QTableWidgetItem(sqm.record(i).value("Qualityname").toString()));
    ...
    int c=sqm.rowCount();
    tw->setItem(c,0,new QTableWidgetItem(QString("*Q%1").arg(getfreeNo())));
    tw->setItem(c,3,new QTableWidgetItem("新"));
    tw->selectRow(c);

}

void QuTab::readytoaddnew()
{
    //page 1
...
void QuTab::on_tableWidget_2_itemSelectionChanged()
{
    ...
    QSqlQueryModel sqm;
    db.getQubyid(sqm,::g_current_index,no);
    //    SetPos();
    ...
    else
    {
        for(int i=0;i<sqm.rowCount();i++)
        {
            ...
            QString cname=sqm.record(i).value("Qualityname").toString();
            if(i==0)
            {
                SetPos();
...
void QuTab::on_sr_yes_clicked()
{
    reg();
    initstate();
}

void QuTab::initstate()
{
    //page 1
    ui->comboBox_7->setCurrentIndex(0);
    //不可编辑
    ...
    ui->lineEdit_7->setText(QString("Q%1").arg(getfreeNo()));
    ui->sr_yes->setEnabled(false);
}

void QuTab::on_comboBox_5_currentIndexChanged(int index)
{
    ui->sr_yes->setEnabled(true);
}

void QuTab::on_comboBox_11_currentIndexChanged(int index)
{
    ui->sr_yes->setEnabled(true);
}

void QuTab::on_comboBox_6_currentIndexChanged(int index)
{
    ui->sr_yes->setEnabled(true);
}
...
void QuTab::on_lineEdit_7_textChanged(const QString &arg1)
{
    ...
    {
        ui->tableWidget_2->item(num-1,0)->setText(QString("*Q%1").arg(getfreeNo()));
...
void QuTab::on_sr_del_clicked()
{
    ...
    if(QMessageBox::Ok==box.exec ())
    {
        ...
        if(items.size()==0) return;
        QString no=items.at(0)->text();
        db.delQubyid(::g_current_index,no);
        refreshtable();
    }
