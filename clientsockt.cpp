...
int  CCliSocket::Send(unsigned char* sendbuf, int len)
{
	/*int ret = 0;
	do
	{
	...
	return ret == len;
	*/
    return  send(clisockfd,(char*) sendbuf, len, 0);
}

int  CCliSocket::Recv(unsigned char* recvbuf, int len)
{

        return  recv(clisockfd, (char*)recvbuf, len, 0);
}
